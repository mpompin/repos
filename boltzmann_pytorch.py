...
def convert(data):
    new_data = []
    for id_user in np.arange(1,nb_users+1):
        id_movies = data[data[:,0] == id_user][:,1]
        id_ratings = data[data[:,0] == id_user][:,2]
        ratings = np.zeros(shape=nb_movies)
...
for epoch in np.arange(1,nb_epoch + 1):
    train_loss = 0
    s = 0.
    for id_user in np.arange(0,nb_users - batch_size,batch_size):
...
test_loss = 0.
s = 0.
for id_user in np.arange(0,nb_users):
    v = training_set[id_user:id_user + 1]
    vt = test_set[id_user:id_user + 1]
    if len(vt[vt>=0]) > 0 :
        _, h = rbm.sample_h(v)
        _, v = rbm.sample_v(h)
        test_loss += torch.mean(torch.abs(vt[vt >= 0] - vt[vt>=0]))
    s += 1.
